...
function (Coords, Bandwidth, x, WType = "Binary") 
{
    ...
    for (i in 1:Obs) {
        ...
        for (j in 1:Obs) {
            ...
            if (j != i) {
                moran_nom <- moran_nom + Wts[i, j] * (x[i] - 
                  mean.x) * (x[j] - mean.x)
            }
        }
        moran_denom <- moran_denom + ((x[i] - mean.x) * (x[i] - 
            mean.x))
    }
    diag(Wts) <- 0
    sum.w = sum(Wts)
    ...
    Results <- list(W = Wts, Morans.I = morans.I, Expected.I = E.I, 
        z.resampling = Z.I.resampling, z.randomization = Z.I.randomization, 
